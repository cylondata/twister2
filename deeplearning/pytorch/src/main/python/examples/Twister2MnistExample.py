...
env = Twister2Environment(resources=[{"cpu": 1, "ram": 512, "instances": PARALLELISM}], mpi_aware=True)
...
def read_train_tuples(itr, collector, ctx: TSetContext):
    for index, data in enumerate(itr):
        ...
        comm.send(index, dest=index, tag=index + 10)
...
def read_test_tuples(itr, collector, ctx: TSetContext):
    for index, data in enumerate(itr):
        ...
        comm.send(index, dest=index, tag=index + 20)
...
source_train_data_cache = source_train.compute(read_train_tuples).cache()

source_test = env.create_source(DataSource(train=False), PARALLELISM)
source_test_data_cache = source_test.compute(read_test_tuples).cache()
...
if train_data:
    train_data_seg = comm.recv(source=world_rank, tag=world_rank + 10)
    print("Rank {} : Train Data Received {}".format(world_rank, train_data_seg.shape))
else:
    test_data_seg = comm.recv(source=world_rank, tag=world_rank + 20)
    print("Rank {} : Test Data Received {}".format(world_rank, train_data_seg.shape))