...
def read_train_tuples(itr, collector, ctx: TSetContext):
    ...
    collector.collect(train_data.tolist())
    print("Train Tuples ", train_data.shape, train_target.shape)


def read_test_tuples(itr, collector, ctx: TSetContext):
    test_data = next(itr)
    test_target = next(itr)
    collector.collect(test_data.tolist())
    print("Test Tuples", test_data.shape, test_target.shape)
...
# comm.Send([send_data, MPI.DOUBLE], dest=world_rank, tag=world_rank)
# print("Rank {} Send Data {}".format(world_rank, send_data.shape))
#
#
# data = np.empty(785, dtype='double')
# comm.Recv([data, MPI.DOUBLE], source=world_rank, tag=world_rank)
# print("Rank {} Reveived Data {}".format(world_rank, data.shape))
