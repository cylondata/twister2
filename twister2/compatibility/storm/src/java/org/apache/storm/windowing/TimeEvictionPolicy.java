...
public class TimeEvictionPolicy<T> implements EvictionPolicy<T, EvictionContext> {
  ...
  public Action evict(Event<T> event) {
    long now = evictionContext == null
        ? System.currentTimeMillis() : evictionContext.getReferenceTime();
    ...
    if (context.getSlidingInterval() != null) {
      if (prevContext == null) {
        ...
        if (Math.abs(delta) > 100) {
          LOG.warning(() -> String.format("Possible clock drift or long running "
                  + "computation in window; "
