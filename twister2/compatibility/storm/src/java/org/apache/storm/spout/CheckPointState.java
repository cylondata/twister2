 ...
 * </pre>
 * <p>
 * During recovery, if a previous transaction is in PREPARING state, it is rolled back since all
 * bolts in the topology might not have
 * prepared (saved) the data for commit. If the previous transaction is in COMMITTING state, it
 * is rolled forward (committed) since some
 * bolts might have already committed the data.
 * <p>
 * During normal flow, the state transitions from PREPARING to COMMITTING to COMMITTED. In case of
 * failures the prepare/commit operation is
 * retried.
 * </p>
 */
public class CheckPointState {
    ...
    switch (state) {
      case PREPARING:
        nextState = recovering
            ? new CheckPointState(txid - 1, COMMITTED) : new CheckPointState(txid, COMMITTING);
        break;
      case COMMITTING:
  ...
  public enum State {
    ...
    COMMITTING,
    /**
     * The checkpoint spout has started preparing the transaction for commit and the prepare
     * is in progress.
     */
    PREPARING
  }
