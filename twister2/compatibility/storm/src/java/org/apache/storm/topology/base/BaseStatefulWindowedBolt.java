...
public abstract class BaseStatefulWindowedBolt<T extends State>
    extends BaseWindowedBolt implements IStatefulWindowedBolt<T> {
   ...
   * This is used to track the windowing boundaries and
  ...
  /**
   * If set, the stateful windowed bolt would use the backend state for window
   * persistence and only keep a sub-set of events in memory as
  ...
  /**
   * The maximum number of window events to keep in memory. This is meaningful
   * only if {@link #withPersistence()} is also set. As the
   * number of events in memory grows close to the maximum, the events that are less
   * likely to be used again are evicted and persisted.
   * The default value for this is {@code 1,000,000}.
   *
   * @param maxEventsInMem the maximum number of window events to keep in memory
   */
  public BaseStatefulWindowedBolt<T> withMaxEventsInMemory(long maxEventsInMem) {
    this.maxEventsInMemory = maxEventsInMem;
    return this;
  }
  ...
  public long maxEventsInMemory() {
    return maxEventsInMemory > 0
        ? maxEventsInMemory : IStatefulWindowedBolt.super.maxEventsInMemory();
  }
