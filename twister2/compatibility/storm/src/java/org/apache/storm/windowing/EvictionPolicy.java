...
/**
 * Eviction policy tracks events and decides whether an event should be evicted from
 * the window or not.
 *
...
public interface EvictionPolicy<T, S> {
   ...
   * @return the {@link org.apache.storm.windowing.EvictionPolicy.Action} to be taken
   * based on the input event
   */
  Action evict(Event<T> event);

  /**
   * Tracks the event to later decide whether {@link EvictionPolicy#evict(Event)}
