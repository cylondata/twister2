...
public class WatermarkTimeTriggerPolicy<T> implements TriggerPolicy<T, Long> {
  ...
  /**
   * Invokes the trigger all pending windows up to the watermark timestamp. The end ts of
   * the window is set in the eviction policy context
    ...
    while (windowEndTs <= watermarkTs) {
      ...
      if (handler.onTrigger()) {
        ...
        if (ts == Long.MAX_VALUE) {
          LOG.fine(String.format("No events to process between %s and watermark ts %s",
