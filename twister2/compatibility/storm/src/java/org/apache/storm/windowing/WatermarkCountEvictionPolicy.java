...
/**
 * An eviction policy that tracks count based on watermark ts and evicts events up to the
 * watermark based on a threshold count.
 *
...
public class WatermarkCountEvictionPolicy<T> implements EvictionPolicy<T, Pair<Long, Long>> {
  ...
  public Action evict(Event<T> event) {
    if (getContext() == null) {
      //It is possible to get asked about eviction before we have a context, due
      // to WindowManager.compactWindow.
