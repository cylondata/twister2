...
/**
 * A trigger policy that tracks event counts and sets the context for eviction policy to
 * evict based on latest watermark time.
 *
...
public class WatermarkCountTriggerPolicy<T> implements TriggerPolicy<T, Long> {
  ...
  /**
   * Triggers all the pending windows up to the waterMarkEvent timestamp based on the
  ...
  private void handleWaterMarkEvent(Event<T> waterMarkEvent) {
    ...
    List<Long> eventTs = windowManager.getSlidingCountTimestamps(lastProcessedTs,
        watermarkTs, count);
    for (long ts : eventTs) {
