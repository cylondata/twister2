...
public class StatefulWindowManager<T> extends WindowManager<T> {
  private static final Logger LOG = Logger.getLogger(StatefulWindowManager.class.getName());
  ...
  private static <T> Iterator<T> expiringIterator(Iterator<T> inner, IteratorStatus status) {
    return new Iterator<T>() {
      ...
      public boolean hasNext() {
        ...
        throw new IllegalStateException("Stale iterator, the iterator is "
            + "valid only within the corresponding execute");
      }
  ...
  public boolean onTrigger() {
    ...
    if (hasEvents) {
      ...
      LOG.fine("invoking windowLifecycleListener onActivation with iterator");
      // reuse the retrieved iterator
      Supplier<Iterator<T>> wrapper = new Supplier<Iterator<T>>() {
        private Iterator<T> initial = it;
        ...
        public Iterator<T> get() {
          ...
          throw new IllegalStateException("Stale window, the window is "
              + "valid only within the corresponding execute");
        }
      };
      windowLifecycleListener.onActivation(wrapper, null,
          null, evictionPolicy.getContext().getReferenceTime());
