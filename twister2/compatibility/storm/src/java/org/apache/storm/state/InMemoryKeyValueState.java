...
public class InMemoryKeyValueState<K, V> implements KeyValueState<K, V> {
  private static final Logger LOG = Logger.getLogger(InMemoryKeyValueState.class.getName());
  ...
  public void prepareCommit(long txid) {
    LOG.fine(() -> String.format("prepare commit, txid %d", txid));
  ...
  public void commit(long txid) {
    LOG.fine(() -> String.format("commit, txid %d", txid));
    if (preparedState != null && txid == preparedState.txid) {
