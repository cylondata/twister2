...
public abstract class BaseWindowedBolt implements IWindowedBolt {
  ...
  //for twister2 implementation we are removing transient
  protected final Map<String, Object> windowConfiguration;
   ...
   * If this field is not present in the incoming tuple, an
   * {@link IllegalArgumentException} will be thrown. The field MUST contain a timestamp
   * in milliseconds
   *
   ...
   *
   * @param timestampExtr the {@link TimestampExtractor} implementation
   */
  public BaseWindowedBolt withTimestampExtractor(TimestampExtractor timestampExtr) {
    ...
    if (this.timestampExtractor != null) {
      throw new IllegalArgumentException("Window is already configured with a "
          + "timestamp extractor: " + timestampExtr);
    }
  ...
  /**
   * Specify a stream id on which late tuples are going to be emitted. They are going to
   * be accessible via the {@link
  ...
  /**
   * Specify the maximum time lag of the tuple timestamp in milliseconds. It means that the
   * tuple timestamps cannot be out of order by
  ...
  /**
   * Specify the watermark event generation interval. For tuple based timestamps, watermark
   * events are used to track the progress of time
   *
