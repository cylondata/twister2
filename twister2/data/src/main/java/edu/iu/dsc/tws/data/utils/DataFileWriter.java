...
public class DataFileWriter {
  ...
  //private org.apache.memory.RootAllocator rootAllocator = null;

  private volatile FSDataInputStream fdis;

  public DataFileWriter(Config cfg, String filename) {
    this.config = cfg;
   ...
   */
  public double[][] writeData(Path path, int dimension, int datasize) {
    ...
    try {
      ...
      //VectorSchema vectorSchema = VectorSchemaRoot.create(arrowSchema, this.rootAllocator);

      FileInputStream fileInputStream = new FileInputStream(new File("/tmp/input.arrow"));
      fileInputStream.getChannel();
      if (DataContext.TWISTER2_HDFS_FILESYSTEM.equals(fileName)) {
