...
public abstract class CompleteArrowInputPartitioner<OT>
  ...
  public CompleteArrowInputPartitioner(Path filePath, Config cfg) {
    this.filePath = filePath;
    this.config = cfg;
  }

  public CompleteArrowInputPartitioner(Path filePath, Config cfg, int datasize) {
    this.filePath = filePath;
    this.config = cfg;
    ...
    for (final FileStatus file : files) {
      ...
      if (len > 0) {
        ...
        for (int i = 0; i < curminNumSplits; i++) {
          ...
          final FileInputSplit fis = new CSVInputSplit(splitNum++, file.getPath(), position,
              splitSize, blocks[blockIndex].getHosts());
          inputSplits.add(fis);
        }
      } else {
