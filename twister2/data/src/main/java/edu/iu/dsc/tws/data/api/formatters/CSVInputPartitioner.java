...
public abstract class CSVInputPartitioner<OT> implements InputPartitioner<OT, FileInputSplit<OT>> {
  ...
  public FileInputSplit<OT>[] createInputSplits(int minNumSplits) throws IOException {
   ...
   /* if (pathFile.isDir()) {
    ...
    for (final FileStatus file : files) {
      ...
      long[] splitSizes = getSplitSizes(file.getPath(), curminNumSplits, splSize);
  ...
  private long[] getSplitSizes(Path filename, int numberOfSplits, int splitSize)
      throws IOException {
    ...
    boolean skipLf = true;
    int overflow = -1;
    LOG.info("number of splits and split size:" + numberOfSplits + "\t" + splitSize);
    outerfor:
