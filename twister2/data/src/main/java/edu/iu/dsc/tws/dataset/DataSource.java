...
public class DataSource<T, O extends InputSplit<T>> extends DataObjectImpl<T> {
  ...
  public DataSource(Config cfg, InputPartitioner<T, O> input, int numSplits) {
    ...
    try {
      ...
      throw new Twister2RuntimeException("Failed to create the input splits because, it %s", e);
    }
  }
  ...
  public InputSplit<T> getNextSplit(int id) {
    ...
    if (split != null) {
      try {
        ...
        throw new Twister2RuntimeException("Failed to open the input split because, it %s", e);
      }
      return split;
    } else {
      return null;
