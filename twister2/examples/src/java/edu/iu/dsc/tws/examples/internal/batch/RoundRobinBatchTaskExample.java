...
public class RoundRobinBatchTaskExample implements IWorker {
  ...
  private static class SourceTask1 extends BaseBatchSourceTask {
    ...
    public void execute() {
      if (count == 999) {
        if (ctx.writeEnd("partition-edge1", "Hello")) {
          count++;
        }
      } else if (count < 999) {
        if (ctx.write("partition-edge1", "Hello")) {
          count++;
        }
      }
    }
  ...
  private static class SinkTask1 extends BaseBatchSinkTask {
  ...
  private static class SinkTask2 extends BaseBatchSinkTask {
    private static final long serialVersionUID = -254264903510284798L;
    private int count = 0;
