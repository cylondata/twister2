...
public class LoadBalanceBatchTask implements IContainer {
  @Override
  public void init(Config config, int id, ResourcePlan resourcePlan) {
    ...
    builder.connect("source", "sink", "load-balance-edge",
        CommunicationOperationType.BATCH_LOAD_BALANCE);
  ...
  private static class RecevingTask extends SinkBatchTask {
    ...
    public boolean execute(IMessage message) {
      if (count % 1000000 == 0) {
        System.out.println("Message Load Balanced : " + message.getContent()
            + ", Count : " + count);
      }
      count++;
      return true;
  ...
  public static void main(String[] args) {
    ...
    jobBuilder.setContainerClass(LoadBalanceBatchTask.class.getName());
