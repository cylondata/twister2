...
#wget http://149.165.150.81:8082/twister2/mesos/twister2-core-0.3.0.tar.gz .
...
if [ $method = "LOCAL" ]; then
  ...
  cp $11/twister2-core-0.3.0.tar.gz .
else
  wget $8 .
  wget $9 .
fi

tar -xvf twister2-core-0.3.0.tar.gz
...
if [ $NOMAD_ALLOC_INDEX = "0" ] && [ $1 = "false" ]; then
  java $debug $profile -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=/tmp -Djava.util.logging.config.file=nomad/logger.properties -cp $cp edu.iu.dsc.tws.rsched.schedulers.nomad.NomadJobMasterStarter --container_class $4 --job_name $5 --job_id $7 --twister2_home . --cluster_type nomad --config_dir . 2>&1 | tee out.txt
else
