#!/usr/bin/env bash

em="\"\""
cp=""
if [ $em != $2 ]; then
    cp=$2
fi

# debug="-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=5005"
# echo mpirun -np $1 --hostfile $8 java -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=/tmp -Djava.util.loggi.config.file=common/logger.properties $cp -cp $3 edu.iu.dsc.tws.rsched.schedulers.standalone.MPIWorker --container_class $4 --job_id $5 --twister2_home $6 --cluster_type standalone --config_dir $7

if [ "$OMPI_COMM_WORLD_RANK" = "0" ]; then
    profile=-agentpath:/home/skamburu/tools/jprofiler10/bin/linux-x64/libjprofilerti.so=port=8849
    debug=
    #if 15th arg is debug them enable debug
    if [ "${15}" = "debug" ]; then
        debug=-agentlib:jdwp=transport=dt_socket,server=y,suspend=y,address=5006
    fi
fi

profile=

# set JAVA_HOME by user
CUSTOM_JAVA_HOME=

# to set LD library path
# LD_LIBRARY_PATH=

# check if config specified JAVA_HOME
if [ -z "${CUSTOM_JAVA_HOME}" ]; then
    # config did not specify JAVA_HOME. Use system JAVA_HOME
    CUSTOM_JAVA_HOME=${JAVA_HOME}
fi
# check if we have a valid JAVA_HOME and if java is not available
if [ -z "${CUSTOM_JAVA_HOME}" ] && ! type java > /dev/null 2> /dev/null; then
    echo "Please specify JAVA_HOME. in conf/standalone/exp.sh or as system-wide JAVA_HOME."
    exit 1
else
    JAVA_HOME=${CUSTOM_JAVA_HOME}
fi

# set the lib path to bundled mpi, if that mpirun is used
lib_path="-Djava.library.path=${LD_LIBRARY_PATH}"
if [ "$9" = "${17}/ompi/bin/mpirun" ]; then
  lib_path="${lib_path}:${17}/ompi/lib"
fi

illegal_access_warn=
if [ "${16}" = "suppress_illegal_access_warn" ]; then
  illegal_access_warn="--illegal-access=deny"
fi

# echo "---mpicommand: " $9 -np ${1} --map-by ${14} --hostfile ${8} $JAVA_HOME/bin/java $illegal_access_warn $lib_path $debug $profile -Djava.util.logging.config.file=common/logger.properties ${10} ${11} $cp -cp ${3}:${17}/lib edu.iu.dsc.tws.rsched.schedulers.standalone.MPIWorkerStarter --container_class $4 --job_id $5 --twister2_home $6 --cluster_type standalone --config_dir $7 --job_master_port ${12} --job_master_ip ${13} --restore_job ${18} --restart_count ${19}
$9 -np ${1} --map-by ${14} --hostfile ${8} $JAVA_HOME/bin/java $illegal_access_warn $lib_path $debug $profile -Djava.util.logging.config.file=common/logger.properties ${10} ${11} $cp -cp ${3}:${17}/lib edu.iu.dsc.tws.rsched.schedulers.standalone.MPIWorkerStarter --container_class $4 --job_id $5 --twister2_home $6 --cluster_type standalone --config_dir $7 --job_master_port ${12} --job_master_ip ${13} --restore_job ${18} --restart_count ${19}
