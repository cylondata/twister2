...
public class RowItrComputeCollectorOp extends BaseComputeOp<Iterator<Table>> {
  ...
  private class ManyTableIterator implements Iterator<Row> {
    private Iterator<Row> currentIterator = null;
    private Iterator<Table> tableIterator;

    ManyTableIterator(Iterator<Table> tableIterator) {
      this.tableIterator = tableIterator;
      if (tableIterator.hasNext()) {
        this.currentIterator = tableIterator.next().getRowIterator();
      }
    }
    ...
    public boolean hasNext() {
      if (currentIterator == null || !currentIterator.hasNext()) {
        if (tableIterator.hasNext()) {
          ...
          currentIterator = next.getRowIterator();
        } else {
