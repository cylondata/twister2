...
public final class ZKRestartCheck {
  ...
  /**
   * Create job znode for worker restart checking
   * Assumes that there is no znode exists in the ZooKeeper
   * This method should be called by the submitting client
   */
  public static void createZNode(CuratorFramework client, String rootPath, String jobName)
      throws Exception {
  ...
  public static boolean isWorkerRestarting(CuratorFramework client,
    ...
    if (client.checkExists().forPath(workerCheckPath) != null) {
      LOG.warning("WorkerRestartCheckPath exists: " + workerCheckPath);
      return true;
    }
    ...
    try {
      client
          ...
          .creatingParentsIfNeeded()
          .withMode(CreateMode.PERSISTENT)
          .forPath(workerCheckPath);

      LOG.info("WorkerRestartCheckPath created: " + workerCheckPath);
      return false;

    } catch (Exception e) {
      throw new Exception("WorkerRestartCheckPath can not be created: " + workerCheckPath, e);
    }
  }
