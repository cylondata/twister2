...
public class ZKJobsMonitor {
  ...
  private PathChildrenCache jobChildrenCache;

  public ZKJobsMonitor(Config config) {
    this.config = config;
  }
  ...
  public boolean initialize() throws Exception {
    ...
    try {
      ...
      jobChildrenCache = new PathChildrenCache(client, ZKContext.rootNode(config), false);
      jobChildrenCache.start();

      addListener(jobChildrenCache);
    ...
    PathChildrenCacheListener listener = new PathChildrenCacheListener() {
      ...
      public void childEvent(CuratorFramework clientOfEvent, PathChildrenCacheEvent event) {
        ...
        switch (event.getType()) {
          case CHILD_ADDED:
            String jobName = ZKPaths.getNodeFromPath(event.getData().getPath());
            try {
