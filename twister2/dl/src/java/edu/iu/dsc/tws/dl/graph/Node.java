...
public class Node<K> implements Serializable {
   ...
   * @return another node
   */
  public Node<K> add(Node<K> node, Edge e){
    if(e == null){
      e = new Edge();
    }
    if (!node.prevs.contains(new NodeEdgePair<K>(this, e))) node.prevs.add(new NodeEdgePair<K>(this, e));
    if (!this.nexts.contains(new NodeEdgePair<K>(node, e))) this.nexts.add(new NodeEdgePair<K>(node, e));
    return node;
  }

  public void addPrevious(Node<K> node, Edge e){
    if(e == null){
      e = new Edge();
    }
    if (!this.prevs.contains(new NodeEdgePair<K>(node, e))) this.prevs.add(new NodeEdgePair<K>(node, e));
  }

  public void addNexts(Node<K> node, Edge e){
    if(e == null){
      e = new Edge();
    }
    if (!this.nexts.contains(new NodeEdgePair<K>(node, e))) this.nexts.add(new NodeEdgePair<K>(node, e));
  ...
  public Node<K> from(Node<K> node, Edge e){
    if(e == null){
      e = new Edge();
    }
    if (!node.nexts.contains(new NodeEdgePair<K>(this, e))) node.nexts.add(new NodeEdgePair<K>(this, e));
    if (!this.prevs.contains(new NodeEdgePair<K>(node, e))) this.prevs.add(new NodeEdgePair<K>(node, e));
    return node;
  }
  ...
  Node<K> delete(Node<K> node, Edge e){
    if (e != null) {
      if (node.prevs.contains(new NodeEdgePair<K>(this, e))) node.prevs.remove(new NodeEdgePair<K>(this, e));
      if (this.nexts.contains(new NodeEdgePair<K>(node, e))) this.nexts.remove(new NodeEdgePair<K>(node, e));
    } else {
      Node<K> curNode = this;  // Because of the closure
      node.prevs.stream().filter(pair -> pair.getValue0() == curNode).forEach(k -> node.prevs.remove(k));
      this.nexts.stream().filter(pair -> pair.getValue0() == node).forEach(k -> this.nexts.remove(k));
    }
    return this;
  }
