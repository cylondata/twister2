...
public class DenseTensorBLAS {
  ...
  double[] a,int aOffset,int lda, double[] b,int bOffset,int ldb,double  beta,
  ...
  public static void gemv(double alpha, Tensor matrix,Tensor vector,double beta,Tensor r){
    ...
    if (matrix.stride(1) == 1) {
      TensorNumeric.gemv('N', matrix.size(1), matrix.size(2), alpha, matrix.storage().toDoubleArray(),
          matrix.storageOffset() - 1,
          matrix.stride(2), vector.storage().toDoubleArray(), vector.storageOffset() - 1, vector.stride(1),
          beta, r.storage().toDoubleArray(),
          r.storageOffset() - 1, r.stride(1));
    } else if (matrix.stride(2) == 1) {
      TensorNumeric.gemv('T', matrix.size(2), matrix.size(1), alpha, matrix.storage().toDoubleArray(),
          matrix.storageOffset() - 1,
          matrix.stride(1), vector.storage().toDoubleArray(), vector.storageOffset() - 1,
      ...
      TensorNumeric.gemv('T', mat.size(2), mat.size(1), alpha, mat.storage().toDoubleArray(), mat.storageOffset() - 1,
          mat.stride(1), vector.storage().toDoubleArray(), vector.storageOffset() - 1, vector.stride(1),
          beta, r.storage().toDoubleArray(),
          r.storageOffset() - 1, r.stride(1));
    }
  }
